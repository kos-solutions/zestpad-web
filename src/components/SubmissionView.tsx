...
import { SaveIndicator, ErrorBanner, Modal, Spinner, StatusChip } from './ui';
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <Link href={backHref} className="text-sm text-ink-500 hover:text-ink-800">
            ← {role.isTeacher ? 'Lucrări' : lesson.topicTitle}
          </Link>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {role.isOwner && !locked && (
            <button onClick={submitHomework} className="btn-primary" disabled={busy}>
              {busy && <Spinner />} Predă tema
            </button>
          )}
          {role.isTeacher && submission.status !== 'NOT_STARTED' && (
            <button onClick={() => setGradeOpen(true)} className="btn-primary" disabled={busy}>
      ...
      {role.isOwner && locked && (
        <div className="rounded-lg border border-zest-200 bg-zest-50 px-4 py-2.5 text-sm text-zest-900">
          Ai predat tema
      ...
      {role.isTeacher && (
        <div className="rounded-lg border border-ink-200 bg-ink-50 px-4 py-2.5 text-sm text-ink-700">
          Scrisul elevului e dedesubt. Ce scrii tu apare cu roșu, ca strat separat — nu îi modifici lucrarea.
        </div>
      )}

      {submission.comment && (
        <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-green-800">Observația profesorului</p>
          <p className="mt-1 text-sm text-green-900">{submission.comment}</p>
        </div>
      )}
      ...
      <ZestCanvas
        ref={canvasRef}
        value={content}
        // Profesorul scrie corectura; sub el vede lucrarea elevului.
        underlay={role.isTeacher ? submission.content : lesson.prompt}
        underlayColor={role.isTeacher ? undefined : '#8494ae'}
        // Elevul vede corectura profesorului deasupra, cu rosu.
        overlay={!role.isTeacher && submission.status === 'GRADED' ? submission.feedback : null}
        overlayColor="#dc2626"
        background={lesson.background}
        readOnly={!canEdit}
        defaultColor={role.isTeacher ? '#dc2626' : '#1a1a1a'}
      ...
      <Modal open={gradeOpen} title="Notează lucrarea" onClose={() => setGradeOpen(false)}>
        <form onSubmit={saveGrade} className="space-y-4">
          <div>
            ...
            <input
              id="grade" className="input" autoFocus value={grade}
              onChange={(e) => setGrade(e.target.value)} placeholder="ex: 9 sau FB"
            />
          </div>
          <div>
            <label className="label" htmlFor="comment">Observație (opțional)</label>
            <textarea
              id="comment" className="input min-h-[90px]" value={comment}
